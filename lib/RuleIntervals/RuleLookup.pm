use RuleIntervals::node;
package RuleIntervals::RuleLookup;
...
sub new {
	...
	$self->{root} = RuleIntervals::node->new(%{$args{root}}) if exists $args{root};
	$self->{nodes} = 
		[ map { 
			RuleIntervals::node->new(%$_)
		} @{$args{nodes}} ]
...
sub root {
	my ($self, $val) = @_;
	$val = RuleIntervals::node->new(%$val) if defined $val and not UNIVERSAL::can($val, 'can'); # bless it if not yet blessed
	return @_ > 1 ? $self->{root} = $val : $self->{root};
}
sub nodes { 
	@_ > 1 ? $_[0]{nodes} = 
		[ map { 
			(ref and not UNIVERSAL::can($_, 'can')) ? RuleIntervals::node->new(%$_) : $_
		} @{$_[1]} ]
	 : $_[0]{nodes}
}
...
sub deserialize {
	...
	$self->{root} = RuleIntervals::node->deserialize($data, $object_offset + $offsets[0]) if $offsets[0] != 0;
	$self->{nodes} = $self->deserialize_array('[RuleIntervals::node]', $data, $object_offset + $offsets[1]) if $offsets[1] != 0;
...
sub serialize {
	...
	if (not defined $cache) {
		...
		my $vtable = $self->serialize_vtable(
			defined $self->{root} ? RuleIntervals::node->flatbuffers_struct_length : 0,
		...
		if (defined $self->{nodes}) {
			my ($array_object, @array_objects) = $self->serialize_array('[RuleIntervals::node]', $self->{nodes}, $cache);
