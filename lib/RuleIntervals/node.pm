use RuleIntervals::rule;
package RuleIntervals::node;
...
sub rules { 
	@_ > 1 ? $_[0]{rules} = 
		[ map { 
			(ref and not UNIVERSAL::can($_, 'can')) ? RuleIntervals::rule->new(%$_) : $_
		} @{$_[1]} ]
	 : $_[0]{rules}
}
...
sub deserialize {
	...
	$self->{rules} = $self->deserialize_array('[RuleIntervals::rule]', $data, $offset + 16);
...
sub serialize {
	...
	$data .= pack 'l<', $self->{min_time} // die 'struct RuleIntervals::node requires field min_time';
	$data .= pack 'l<', $self->{min_node} // die 'struct RuleIntervals::node requires field min_node';
	$data .= pack 'l<', $self->{max_time} // die 'struct RuleIntervals::node requires field max_time';
	$data .= pack 'l<', $self->{max_node} // die 'struct RuleIntervals::node requires field max_node';
	do {
		my ($array_object, @array_objects) = $self->serialize_array('[RuleIntervals::rule]', $self->{rules}, $cache);
